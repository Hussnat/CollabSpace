...
// --- Sidebar ---
const Sidebar = ({ collapsed, mobileOpen, onClose, onOpenInvite }) => {
  const links = [
      ...
      <div className="mt-auto pt-6 flex flex-col gap-2">
        ...
        <div className={`flex gap-2 ${collapsed ? "justify-center" : ""}`}>
          <button
            ...
            className="px-3 py-2 w-full text-center bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 shadow-lg transition"
          >
            {!collapsed ? "Invite" : <User size={18} />}
          </button>
        </div>
<div className={`mt-6 flex flex-col gap-2 ${collapsed ? "items-center" : ""}`}>
  <button
    ...
    className="flex items-center gap-2 p-2 rounded-lg hover:bg-indigo-50 dark:hover:bg-slate-700 transition text-sm"
  >
    <LogOut size={18} />
    {!collapsed && "Logout"}
  </button>
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100">
      <Sidebar collapsed={collapsed} mobileOpen={mobileOpen} onClose={() => setMobileOpen(false)} onOpenInvite={() => setInviteOpen(true)} />
      ...
      <Modal open={inviteOpen} onClose={() => setInviteOpen(false)} title="Invite Team Member">
        <form onSubmit={handleInvite} className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-3">
            <button type="button" onClick={() => setInviteOpen(false)} className="px-4 py-2 rounded-lg border">Cancel</button>
            <button type="submit" className="px-4 py-2 rounded-lg bg-indigo-600 text-white">Send Invite</button>
          </div>
        </form>
      </Modal>
    </div>
